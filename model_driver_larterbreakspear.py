...
class Driver_Setup:
  ...
  def parse_args(self):  # {{{
    ...
    parser.add_argument('-s3', '--n_sweep_arg3', default=1, help='num grid points for 3st parameter', type=int)
    parser.add_argument('-s4', '--n_sweep_arg4', default=1, help='num grid points for 3st parameter', type=int)
    parser.add_argument('-s5', '--n_sweep_arg5', default=2, help='num grid points for 3st parameter', type=int)
    parser.add_argument('-s6', '--n_sweep_arg6', default=0, help='num grid points for 3st parameter', type=int)
    ...
    parser.add_argument('-m', '--model', default='larter_breakspear', help="neural mass model to be used during the simulation")
    parser.add_argument('-s', '--states', default=3, type=int, help="number of states for model")
  ...
  def setup_params(self,
    ...
    #        0, 1,  # nmda -.5, 4.0,
    #        1, 1,  # Speed .5, .8,  #
    #        .5, .5,  # Mean Synaptic weight J 12, 16,  #
    #        -.1, -.1,  # Constant parameter to scale the rate of feedback from the firing rate variable to itself -5., -3.,
    #        # .5, 4.
    # ]
    ...
    slh = [
           20, 20,  # G
           0.01, 0.01,  # weight_noise
           .01, 1.51,  # rnmda or 1e-3, 1e-1,  # rnmda = .25 in paper
           3, 3,  # Speed
           0, 5.1,  # aee Excitatory-to-Excitatory Strength # aee = .5 in paper
           # -65e-3, -25e-3,  # VT Excitatory Threshold Mean or -65e-3, -44e-3
           -2.2, 2,  # But is it in mV? VT = -.1 in paper
    ]
    ...
    params = itertools.product(s0, s1, s2, s3, s4, s5)
...
class Driver_Execute(Driver_Setup):
  ...
  def make_kernel(self, source_file, warp_size, args, lineinfo=False, nh='nh'):
    ...
    try:
      with open(source_file, 'r') as fd:
        ...
        mod_func = '_Z17larter_breakspearjjjjjffiiiiiPfS_S_S_S_S_S_S_S_S_S_S_'
  ...
  def run_tvb_rc(self, target_dyn, tearchr_dyn, inject_dyn, inject_dyn_pred, W_out, statesbuf, usebuffer):
    ...
    try:
      for i in tqdm_iterator:
        ...
        # if usebuffer == True and i>int(inject_dyn_pred/self.n_inner_steps) and GO:
  ...
  def run_all(self):
    ...
    # pshift = 3.8 # for h.004 for lorentz for 96 nodes! montbrio
    # pshift = 9.9 # for h.004 for lorentz for 96 nodes! larter for preddelay 50
    pshift =9.9 # for h.004 for lorentz for 96 nodes! larter
    # pshift = 1.3 # for h.004 for lorentz 192 but not good
    dominant_frq = 1.4 #Hz
    sampling_rate = 100 #Hz
    time_shift_seconds = pshift / (2 * np.pi * dominant_frq)
    # self.delay_timesteps = int(round(time_shift_seconds * sampling_rate))
    self.delay_timesteps = 20
    ...
    if notfixpredictiontask == False:
      ...
      fixed_pred_set, teach_pred_set = self.create_task_dyn(phase_shift_pis=pshift, h=stepsizeforLorentz,
    ...
    for tloop in range(self.trainingloops):
      ...
      target_dyn_train, teachr_dyn_train = self.create_task_dyn(phase_shift_pis=pshift, h=stepsizeforLorentz,
                                                                nonzeroffset=nonzeroffset, Sinussus=SinusTask)
      ...
      # nagete to mach membrane potential polarity
      trainer = target_dyn_train[set_dyn, :inject_dyn_ridge] * LorentzScale
      tearchr = teachr_dyn_train[set_dyn, :inject_dyn_ridge] * LorentzScale
      if self.my_rank == 0:
      ...
      if self.trainingloops == 1:
        ...
        plot_target_dyn(trainer, tearchr, "trainer and teacher sig")
      ...
      #   # plot_tavgs(tavg0[:, 1, :, 0], "after train injection V")
        plot_tavgs_notstacked(tavg0[:, 0, :, 0], "after train injection V")
        plot_tavgs_notstacked(tavg0[:, 1, :, 0], "after train injection W")
      ...
      if self.trainingloops == 0:
        plot_target_dyn(scaled_dyn_train[0].T, rolled_dyn_train[0].T, "Input and Teacher b4 torch")
      ...
      if tloop % self.tlsamples == 0 and go_ahead:
        ...
        if notfixpredictiontask == True:
          ...
          target_dyn_pred, teachr_dyn_pred = self.create_task_dyn(phase_shift_pis=pshift, h=stepsizeforLorentz,
                                                                  nonzeroffset=nonzeroffset, Sinussus=SinusTask)

        else:
          # use a fixed prediction set
          # make negative to match polarity of membrane potential of excitatory population
          target_dyn_pred = fixed_pred_set * LorentzScale
          teachr_dyn_pred = teach_pred_set * LorentzScale

        if self.trainingloops == 0:
          plot_target_dyn(target_dyn_pred, teachr_dyn_pred, "fixed one")

        # alter for injection from 250
        tavg1, _ = self.run_tvb_rc(target_dyn_pred[set_dyn, :],
                                teachr_dyn_pred[set_dyn, :], inject_dyn_pred,
    ...
    if self.my_rank == 0:
      ...
      # loosen dfa constraint
      mask_cons_hihgdfa_lowlya = (0.44 < all_result[:, 3]) & (all_result[:, 3] < 0.67)  & (
                all_result[:, 5] <= 0.3) #& (all_result[:, 5] <= 0.2)
      mask_cons_lowdfa_highlya = (0.44 < all_result[:, 3]) & (all_result[:, 3] < 0.67)  & (
                all_result[:, 5] > 0.3) #& (all_result[:, 5] <= 0.2)
      mask_uncon_highdfa_lowlya = (0.12 < all_result[:, 3]) & (all_result[:, 3] < 0.31) & (
                all_result[:, 5] <= 0.3) #& (all_result[:, 5] <= 0.2)
      mask_uncon_lowdfa_highlya = (0.12 < all_result[:, 3]) & (all_result[:, 3] < 0.31) & (
                all_result[:, 5] > 0.3) #& (all_result[:, 5] <= 0.2)
      ...
      # np.save("data/W_out_LB_inc_inout_LB.npy", all_Wouts[best_indices][0])
      #
      # np.save("data/all_p_LB", all_p)
      # np.save("data/all_d_LB", all_d)
      # np.save("data/all_l_LB", all_l)
      # np.save("data/all_m_LB", all_m)

      plotreaction =0
      if plotreaction == 1:
        # inverted_dyn = -target_dyn_train
        # plot_tavgs(tavg1[:,0,:,0], "tavg1[:,0,:,0] ")
        plot_tavgs_notstacked(tavg1[:, 0, :, best_indices[0]], f"prediction res V idx {best_indices[0]} params {paramstoplot[0]}")
        plot_tavgs_notstacked(tavg1[:, 1, :, best_indices[0]], f"prediction res W idx {best_indices[0]} params {paramstoplot[0]}")
    ...
    if plotallpred == True:
      ...
      plot_predictions_vs_true_fullsims(all_Ys[best_indices], all_Y_preds[best_indices], rounded_params)
    ...
    if self.my_rank == 0:
      ...
      self.logger.info('Training rounds: {0:.3g}'.format(self.trainingloops))
